...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['envoyer'], $_POST['nom'])) {
        ...
        $check_query = $connexion->prepare("SELECT COUNT(*) FROM matter WHERE name = ?");
        $check_query->execute([$nom]);
        
        if ($check_query->fetchColumn()) {
            echo "Erreur : Matière déjà existante";
        } else {
            $insert_query = $connexion->prepare("INSERT INTO `matter` (`id`, `name`) VALUES (NULL, :nom)");
            $insert_query->bindParam(':nom', $nom);
            $verif = $insert_query->execute() ; 
            header("Location: ?page=matiere");
            exit();
        }
    } else {
...
<div class="php_content">
    ...
    <div class="bloc_contenu3">
        <div class="gestion_sources">
            <p class="title_exo">Ajouter une matière :</p>
