...
if (isset($_POST['id_suppression'])) {
    ...
    $requete = $connexion->prepare("DELETE FROM matter WHERE id = :id");
...
$requete_total = $connexion->prepare("SELECT COUNT(*) AS total FROM matter;");
...
$requete = $connexion->prepare("SELECT * FROM `matter` LIMIT :offset, :limit;");
...
<div class="php_content">
    ...
    <p class="title_exo">Rechercher des matières</p>
          <p>Rechercher une matière par un nom :</p>
       ...
       <table>
                ...
                <tbody>
                    ...
                    if (isset($_POST['recherche'])) {
                        ...
                        $requete = $connexion->prepare("SELECT * FROM matter WHERE name = :mots");
                            ...
                            echo "<td>
                                    <form method='post'>
                                        <div class='bouton_suppr'>
                                            ...
                                            <a href='?page=modif_matiere&id=" . $nom['id'] . "'>Modifier</a>
                                        </div>
                                    </form>
                                </td>";
                            echo "<td>
                        ...
                        foreach ($donnees as $ligne) {
                            ...
                            echo "<td>
                                <form method='post' action = '?page=modif_matiere'>
                                    <div class='bouton_suppr'>
                                        <input type='hidden' name='id_modif' value='" . $ligne['id'] . "'>
                                        <img src='ico/modifier.svg' alt='Bouton modifier'>&nbsp;
                                        <a href='?page=modif_matiere&id_modif=" . $ligne['id'] . "'>Modifier " . $ligne['id'] . "</a>
                                    </div>
                                </form>
                            </td>";
    ...
    <div class='pagination'>
            <?php
            if ($total_pages > 1) {
                echo "<a href='?page=matiere&num=" . ($page > 1 ? $page - 1 : 1) . "'>&laquo;</a>";
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo "<a href='?page=matiere&num=$i'" . ($page == $i ? " class='active'" : "") . ">$i</a>";
                }
                echo "<a href='?page=matiere&num=" . ($page < $total_pages ? $page + 1 : $total_pages) . "'>&raquo;</a>";
            }
            ?>
    </div>
</div>
