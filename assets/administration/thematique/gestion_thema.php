...
if (isset($_POST['id_suppression'])) {
    ...
    $requete = $connexion->prepare("DELETE FROM thematic WHERE id = :id");
...
$requete_total = $connexion->prepare("SELECT COUNT(*) AS total FROM thematic;");
...
$requete = $connexion->prepare("SELECT * FROM `thematic` LIMIT :offset, :limit;");
...
//SCRIPT PHP SELECT
$requete_all = $connexion->prepare("SELECT * FROM thematic");
$requete_all->execute();
...
<div class="php_content">
    ...
    <p>Rechercher une thématique par un nom :</p>
        <div class="container_one_exo">
            ...
            <p class='title_exo'>Toutes les thématiques</p>
            <table>
                <thead>
                    <th>Nom</th>
                    <th>Modifier</th>
                ...
                <tbody>
                    ...
                    if (isset($_POST['recherche'])) {
                        ...
                        $requete = $connexion->prepare("SELECT * FROM thematic WHERE name = :mots");
                            ...
                            echo "<td>
                                    <form method='post'>
                                        <div class='bouton_suppr'>
                                            ...
                                            <a href='?page=modif_thematic&id=" . $nom['id'] . "'>Modifier</a>
                                        </div>
                                    </form>
                                </td>";
                            echo "<td>
                            ...
                            echo "<td>
                                    <form method='post'>
                                        <div class='bouton_suppr'>
                                            ...
                                            <img src='ico/modifier.svg' alt='Bouton modifier'>&nbsp;
                                            <a href='?page=modif_thematic&id=" . $ligne['id'] . "'>Modifier</a>
                                        </div>
                                    </form>
                                </td>";
                            echo "<td>
    ...
    <div class='pagination'>
            <?php
            if ($total_pages > 1) {
                echo "<a href='?page=thematic&num=" . ($page > 1 ? $page - 1 : 1) . "'>&laquo;</a>";
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo "<a href='?page=thematic&num=$i'" . ($page == $i ? " class='active'" : "") . ">$i</a>";
                }
                echo "<a href='?page=thematic&num=" . ($page < $total_pages ? $page + 1 : $total_pages) . "'>&raquo;</a>";
            }
            ?>
    </div>
</div>
